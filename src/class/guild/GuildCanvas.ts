...
export default class GuildCanvas {
  private guild: Guild;
  private canvasStage: number;
  private size: Size | null;
  private canvas: Canvas | null;
  private uiDatas: CanvasUIDatas.UIDatas | null;
  private prevPixelGridSize: number;
  private newPixelGridSize: number;
  ...
  async update() {
    ...
    if (this.guild.hasFeature("display-pixels")) this.updatePixels();
    if (this.guild.hasFeature("display-player-status")) await this.updatePlayerStatus();
    if (this.guild.hasFeature("display-milestone")) this.updateMilestone();
  ...
  getImage() {
    if (this.canvas === null) return undefined;
    return this.canvas.getImage();
  }

  private getUIData(name: CanvasUIDatas.UINames): CanvasUIDatas.UIData | null {
    if (this.uiDatas === null) return null;
    return this.uiDatas[name];
  }

  private clearUIDataRange(uiData: CanvasUIDatas.UIData) {
    if (this.canvas === null) return;
    this.canvas.clearRange(
      uiData.x,
      uiData.y,
      uiData.width,
      uiData.height
  ...
  private fillPixel(idx: number, pixelSize: number, color: string, pixelsUIData: CanvasUIDatas.UIData, isSeparatorPixel: boolean=false) {
    if (this.canvas === null) return;
  ...
  async updatePlayerStatus() {
    ...
    for (let i = 0; i < Math.min(playerStatuses.length, maxDisplay); i++) {
      ...
      this.canvas.fillStyle = playerStatus.color ?? "#ffffff";

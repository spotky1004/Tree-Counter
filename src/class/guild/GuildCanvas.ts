...
type UINames = "counter" | "pixels" | "playerStatus" | "countMessages";
interface UIData {
  x: number;
  y: number;
  width: number;
...
type UIDatas = Record<UINames, UIData>;
const getUIRatios = (widthPerHeight: number): UIDatas => ({
...
export default class GuildCanvas {
  ...
  private uiDatas: UIDatas;
  private prevPixelGridSize: number;

  constructor(guild: Guild, config: CanvasConfig) {
    const size = config.size ?? getdefaultSize();
  ...
  private static getUIDatas(size: Size): UIDatas {
    const uiRatios = getUIRatios(size.width/size.height);

    const uiData: UIDatas = {} as any;
    let uiName: UINames;
  ...
  getUIData(name: UINames) {
    return this.uiDatas[name];
  }

  clearUIDataRange(uiData: UIData) {
  ...
  private fillPixel(idx: number, pixelSize: number, color: string, pixelsUIData: UIData, isSeparatorPixel: boolean=false) {

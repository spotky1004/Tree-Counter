import type GuildCache from "../class/guild/Guild.js";
...
export type UINames = keyof typeof uiNamesEnum;
...
export type UIDatas = Record<UINames, UIData | null>;

type GetUIRatioFunction = (aspectRatio: number) => UIDatas;
const getUIRatioFunctions: GetUIRatioFunction[] = [
  // Stage 0: nothing
  () => ({
    milestone: null,
    counter: null,
...
export function getCanvasStage(guildCache: GuildCache): number {
...
export function getCanvasSize(stage: number) {
  const canvasSize = canvasSizes[stage];
  if (typeof canvasSize === "undefined") throw new Error("Invaild stage!");
  return canvasSize;
}

export function getUIDatas(size: Size, stage: number) {
  const getUIRatioFunction = getUIRatioFunctions[stage];
  if (typeof getUIRatioFunction === "undefined") throw new Error("Invaild stage!");
  const aspectRatio = Math.max(size.width / size.height, size.height / size.width);
  const uiRatio = getUIRatioFunction(aspectRatio);
  const uiDatas: UIDatas = {} as any;
  let uiName: UINames;

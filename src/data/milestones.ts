...
export type UnlockableFeatures = (typeof unlockableFeaturesEnum)[number];
interface MilestoneItem {
  name: string;
  unlockedFeatures: UnlockableFeatures[];
  countGoal: number;
}
...
const milestones: MilestoneItem[] = [
  ...
  // 20
  {
    countGoal: 99_999_999,
    unlockedFeatures: [],
    name: "Overflow"
  },
];

const milestoneCountGoals: number[] = [];
const milestoneUnlockedFeatures: UnlockableFeatures[][] = [];
...
export function getMilestoneNr(count: number) {
  let nr = -1;
  for (; nr < milestones.length-1; nr++) {
    if (count < milestoneCountGoals[nr+1]) break;
  }
  return nr;
}

export function getUnlockedFeatures(milestoneNr: number) {
  const unlocked: UnlockableFeatures[] = [];
...
export function getMilestone(milestoneNr: number) {
  milestoneNr = Math.max(0, Math.min(milestoneNr, milestones.length-1));
  return milestones[milestoneNr];
}

export function getNextMilestone(count: number) {

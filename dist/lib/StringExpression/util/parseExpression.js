...
export default function parseStringExpression(str, maxLoop = 1000) {
    ...
    while (true) {
        didLoop();
        if (str.match(codeCheckRegexp) === null)
            break;
        let bracketsLevel = 0;
        let beginPos = -1;
        for (let i = 0; i < str.length; i++) {
            ...
            if (char === "{") {
                ...
                bracketsLevel--;
                if (bracketsLevel < 0)
                    throw Error(`Invaild brackets pair. (at ${i})`);
                if (bracketsLevel === 0) {
                    const codeStr = str.slice(beginPos, i + 1);
                    const variableNameStr = (_a = matchOne(codeStr, codeGetRegexp, 1)) !== null && _a !== void 0 ? _a : "";
                    const expressionStr = matchOne(codeStr, codeGetRegexp, 2);
                    if (typeof variableNameStr === "undefined" ||
    ...
    if (invaildTestRegexp2.test(str)) {
        ...
        if (match === null)
            throw Error("Unknown parse error. (003)");
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (char === "(") {
            ...
            testBracketsLevel--;
            if (testBracketsLevel < 0)
                throw Error(`Invaild brackets pair. (at ${i})`);
        }
    }
    if (testBracketsLevel !== 0)
        throw Error("There is one or more unterminated brackets.");
    // Parse functions
    const functions = [];
    ...
    funcLoop: while (true) {
        didLoop();
        if (!functionExistTestRegexp.test(str))
            break;
        let bracketsLevel = 0;
        let beginPos = -1;
        for (let i = 0; i < str.length; i++) {
            ...
            if (char === "(") {
                ...
                if (bracketsLevel === 0) {
                    ...
                    if (typeof funcNameIdx === "undefined")
                        throw Error("Unknown parse error. (004)");
                    const funcName = values[Number(funcNameIdx)];
                    if (typeof funcName === "number")
                        throw Error("Unknown parse error. (005)");
    ...
    function parseExpression(str) {
        if (invaildTestRegexp3.test(str))
            throw Error(`Syntax error (at '${str.slice(-1)}')`);
        while (true) {
            if (!str.includes("(")) {
                parsePart(str);
                break;
    ...
    function parsePart(part) {
        ...
        exprLoop: while (true) {
            for (const regexp of operatorRegexps) {
                ...
                if (typeof operatorPart === "undefined" ||
    ...
    function parseOperator(operator, ...vals) {
        didLoop();
        // Check function
    ...
    function parseFunction(functionData) {
